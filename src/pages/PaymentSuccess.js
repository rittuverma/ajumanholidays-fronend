...
export default function PaymentSuccess() {
  ...
  const paymentId = result.paymentId || result.id || result.orderId || result.order_id || result.razorpay_payment_id;
  const orderId = result.orderId || result.order_id || result.razorpay_order_id || result.id;
  const status = result.status || "success";
  const amount = result.amount || (result.booking && result.booking.amount) || result.totalAmount || 0;
          ...
          {booking && (
            <>
              ...
              <div className="ps-row">
                ...
                <div className="value">{booking.origin} → {booking.destination}</div>
              </div>

              <div className="ps-row">
                <div className="label">Date</div>
                <div className="value">{booking.date}</div>
              </div>

              <div className="ps-row">
                <div className="label">Seats</div>
                <div className="value">{(booking.selectedSeats && booking.selectedSeats.join(", ")) || (booking.seats && booking.seats.toString()) || "—"}</div>
              </div>
            </>
          )}
